...
print(f"Start Time Model Accuracy: {accuracy:.2f}")

# Save the model
model_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'models')
os.makedirs(model_dir, exist_ok=True)
model_path = os.path.join(model_dir, 'start_time_model.pkl')
joblib.dump(model, model_path)
print(f"Start time prediction model saved to {model_path}")